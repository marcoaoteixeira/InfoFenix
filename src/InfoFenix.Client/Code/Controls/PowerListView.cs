...
namespace InfoFenix.Client.Code.Controls {
    ...
    /// </remarks>
    public class PowerListView : ListView {
        ...
        public object DataSource {
            ...
            set {
                ...
                if (value != null && !(value is IList) && !(value is IListSource)) {
                    throw new ArgumentException($"Data source must be {nameof(IList)} or {nameof(IListSource)}");
                }
        ...
        private void LoadColumnsFromSource() {
            ...
            var headers = new ColumnHeader[_properties.Count];
            Columns.Clear();
            for (var column = 0; column < _properties.Count; ++column) {
                var columnName = _properties[column].Name;
        ...
        private void LoadItemsFromSource() {
            ...
            try {
                ...
                var items = _currencyManager.List;

                // Add items to list.
                for (var idx = 0; idx < items.Count; ++idx) {
                    Items.Add(BuildItemForRow(items[idx]));
                }
                var index = _currencyManager.Position;
                if (index != -1) {
        ...
        private ListViewItem BuildItemForRow(object row) {
            var itemText = new string[_properties.Count];
        ...
        private void bindingList_ListChanged(object sender, ListChangedEventArgs e) {
            switch (e.ListChangedType) {
                ...
                case ListChangedType.ItemChanged:
                    var changedRow = _currencyManager.List[e.NewIndex];
                    BeginUpdate();
                ...
                case ListChangedType.ItemAdded:
                    var newRow = _currencyManager.List[e.NewIndex];
        ...
        private void currencyManager_ItemChanged(object sender, ItemChangedEventArgs e) {
            ...
            if (e.Index == -1) {
                ...
                if (!ReferenceEquals(_bindingList, _currencyManager.List)) {
                    _bindingList.ListChanged -= new ListChangedEventHandler(bindingList_ListChanged);
                    if (_currencyManager.List is IBindingList bindingList) {
                        bindingList.ListChanged += new ListChangedEventHandler(bindingList_ListChanged);
                    }
                }
                LoadItemsFromSource();
