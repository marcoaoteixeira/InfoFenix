...
namespace InfoFenix.Entities {
    ...
    public sealed class Document {
        ...
        public string FileName => Path != null ? System.IO.Path.GetFileName(Path) : null;

        #endregion Public Properties

        #region Public Static Methods

        public static Document Map(string filePath) => new Document {
            Code = Common.ExtractCodeFromDocumentFilePath(filePath),
            Payload = File.ReadAllBytes(filePath),
            Path = filePath
        };

        public static Document Map(IDataReader reader) => new Document {
        ...
        public bool Equals(Document obj) => obj != null
